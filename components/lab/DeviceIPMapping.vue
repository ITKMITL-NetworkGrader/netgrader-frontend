              ...
              <Transition name="expand-vertical">
                <div v-if="device.isIsolated" class="space-y-3 pl-8 border-l-2 border-primary/20">
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-2">
                      <Label :for="`ansible-password-${index}`">
                        Ansible Password *
                      </Label>
                      <Input
                        :id="`ansible-password-${index}`"
...
const validationIssues = computed(() => {
  ...
  // Check for isolated device credentials
  props.devices.forEach((device, index) => {
    if (device.isIsolated) {
      if (!device.ansibleUsername?.trim()) {
        issues.push(`${device.deviceName}: Ansible username is required for isolated devices`)
      }
      if (!device.ansiblePassword?.trim()) {
        issues.push(`${device.deviceName}: Ansible password is required for isolated devices`)
      }
    }
  })
